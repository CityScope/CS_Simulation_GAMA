...
global {
	string CITY_GIS_FOLDER <- "./../includes/City/volpe";
	// GIS FILES
	file bound_shapefile <- file(CITY_GIS_FOLDER+"/Bounds.shp");
	file buildings_shapefile <- file(CITY_GIS_FOLDER+"/Buildings.shp");
	file roads_shapefile <- file(CITY_GIS_FOLDER+"/Roads.shp");
	file amenities_shapefile  <- file(CITY_GIS_FOLDER+"/Amenities.shp");
	file table_bound_shapefile <- file(CITY_GIS_FOLDER+"/table_bounds.shp");
	...
	int toggle1;
	int slider1;
	map<int,list> citymatrix_map_settings <- [
		-1::["Green","Green"],
		0::["R","L"],
		1::["R","M"],
		2::["R","S"],
	...
	map<string,rgb> color_map <- [
		...
		"Road"::#black,
		"Park"::#black,
		"Parking"::rgb(50,50,50)
	]; 
	list scale_string <- ["S", "M", "L"];
	list usage_string <- ["R", "O"];
	map<string,int> density_map <- [ // Use for Volpe Site (Could be changed for each city).
		"S"::89,
		"M"::55,
		"L"::15
	];
	...
	action coreInit {
		...
		create building from: buildings_shapefile with: [usage::string(read ("Usage")),
														scale::string(read ("Scale")),
														nbFloors::1+float(read ("Floors"))
		]{
			area <- shape.area;
