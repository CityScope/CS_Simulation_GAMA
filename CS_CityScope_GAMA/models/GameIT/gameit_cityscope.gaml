	...
	init {
		...
		if CityIO {
			create Networking_Client {
				do connect to: "brix" protocol: "websocket_client" port: 8001 with_name: "Client" raw: true;
...
//				do send to: "ws://brix:8001" contents: buildings;
			}
		}
	}
...
species Networking_Client skills: [network] {
	reflex statistics when: CityIO and (cycle mod 2 = 0) {
		...
		do send to: "ws://brix:8001" contents: "[" + layers + "]";
	}
}
