...
species aalto_people skills: [moving] {
	...
	Aalto_buildings living_place <- nil;
	Aalto_buildings working_place <- nil;
	...
	reflex time_to_go_to_work when: current_hour = time_to_work and objective = "resting" {
		...
		if (mode_of_transportation_is_car = true) {
			the_target_parking <- any_location_in(one_of(list_of_available_parking where (each.capacity > 0)));
